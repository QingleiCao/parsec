                                       ...
                                       const char *name)
{
    ...
    for(fl = f->dataflow, di = 0; fl != NULL; fl = fl->next, di++) {
        ...
        if(fl->flow_flags & JDF_FLOW_TYPE_CTL) {
            ...
            coutput("  gpu_task->flow_nb_elts_alloc[%d] = 0;\n", di);
        }else{
            ...
            if(alloc_size_property == NULL){
                coutput("  gpu_task->flow_nb_elts_alloc[%d] = (0 == gpu_task->ec->data[%d].data_in->original->nb_elts_alloc) ? gpu_task->flow_nb_elts[%d] : gpu_task->ec->data[%d].data_in->original->nb_elts_alloc;\n", di, di, di, di);
            }else{
                coutput("  gpu_task->flow_nb_elts_alloc[%d] = %s;\n",
                        di, dump_expr((void**)alloc_size_property->expr, &info));
            }
            coutput("  assert(gpu_task->flow_nb_elts[%d] <= gpu_task->flow_nb_elts_alloc[%d]);\n", di, di);
            coutput("}\n");
        }
